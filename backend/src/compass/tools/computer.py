...
class ComputerTool(BaseAnthropicTool):
    ...
    def handle_text_input(
        self, action: Literal["key", "type"], text: str | None
    ) -> ToolResult:
    ...
    def screenshot(self):
        ...
        try:
            ...
            if not final_path.exists():
                raise ToolError(
                    f"pngquant failed to process image: {optimize_result.error}"
                )
    ...
    def shell(self, command: str, take_screenshot=True) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                output=stdout or None, error=stderr or None, base64_image=base64_image
            )
        except Exception as e:
            raise ToolError(str(e))

    def _handle_mouse_action(
        self, action: str, coordinate: tuple[int, int] | None = None
    ) -> ToolResult:
    ...
    def get_cursor_position(
        self, coordinate: tuple[int, int] | None = None, text: str | None = None
    ) -> ToolResult:
        ...
        if result.output:
            try:
                ...
                raise ToolError(
                    f"Failed to parse cursor position from: {result.output}"
                )
...
def get_screen_dimensions():
    """Get the main display dimensions on macOS & windows.
    ...
    try:
        if platform.system() == "Darwin":  # macOS
            ...
            if match:
                ...
                logger.info(f"Detected screen dimensions on macOS: {width}x{height}")
                return width, height

            logger.warning(
                "Could not parse screen dimensions from system_profiler output"
            )
            return 0, 0
        elif platform.system() == "Windows":  # Windows
            user32 = ctypes.windll.user32
            user32.SetProcessDPIAware()  # Make the process DPI aware
            width = user32.GetSystemMetrics(0)  # Screen width
            height = user32.GetSystemMetrics(1)  # Screen height
            logger.info(f"Detected screen dimensions on windows: {width}x{height}")
            return width, height
