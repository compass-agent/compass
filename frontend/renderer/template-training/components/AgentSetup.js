    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h3>
            {data.isNew ? "Configure" : "Edit"} {data.name}
          </h3>
        ...
        {type === "software" && data.id === "SAP2000" && (
          ...
          >
            ...
            >
              SAP2000 is a structural analysis and design software that the
              agent can use to perform engineering calculations, create models,
          ...
          {type === "tool" && data.id === "fileEditor" && (
            <>
              <div className="form-group">
                ...
                >
                  <label
                    style={{ margin: 0, color: "#ffffff", fontWeight: 500 }}
                  >
                    Root Directory
                  </label>
              ...
              <div style={{ width: "100%" }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      Allows the agent to control the software programmatically
                      through APIs and scripting. This enables automated
                      operations, data processing, and integration with other
                      systems.
                    </div>
                  </div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFormData({
                                    ...
                                    config: {
                                      ...formData.config,
                                      sapAutoAttach: e.target.checked,
                                    },
                                  })
                                }
                                style={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      Enables the agent to interact with the software's user
                      interface like a human user. This includes clicking,
                      typing, reading screen content, and navigating through
                      menus and dialogs.
                    </div>
                  </div>
                      ...
                      <div style={{ marginBottom: 0 }}>
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={
                              formData.config.restrictedToSoftware || false
                            }
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                config: {
                                  ...formData.config,
                                  restrictedToSoftware: e.target.checked,
                                },
                              })
                            }
                            style={{
...
const AgentSetup = ({ onNext, onTrainUI, existingAgent }) => {
  ...
  const removeSoftwareIntegration = (softwareId) => {
    setSoftwareIntegrations(
      softwareIntegrations.filter((s) => s.id !== softwareId)
    )
    setActiveDropdown(null)
  }
  ...
  const handleModalSave = (updatedData) => {
    if (configModalType === "tool") {
      if (updatedData.isNew) {
        ...
        setGeneralTools(
          generalTools.map((t) => (t.id === updatedData.id ? updatedData : t))
        )
      }
              ...
              {showAddToolDropdown && (
                <div className="dropdown-menu">
                  {AVAILABLE_TOOLS.filter(
                    (tool) => !generalTools.find((t) => t.id === tool.id)
                  ).map((tool) => (
                    <div
                      key={tool.id}
                      className="dropdown-item"
                      onClick={() => addGeneralTool(tool.id)}
                    >
                      {tool.name}
                    </div>
                  ))}
                  ...
                  {generalTools.map((tool) => (
                    <div key={tool.id} className="table-row">
                      ...
                      <div className="col configuration">
                        {tool.id === "fileEditor"
                          ? `Root: ${tool.config.rootDir || "Not set"}, ${
                              tool.config.restricted
                                ? "Restricted"
                                : "Full access"
                            }`
                          : `${
                              tool.config.access === "full"
                                ? "Full system access"
                                : "Restricted access"
                            }`}
                      </div>
                        ...
                        <div className="actions-dropdown">
                          ...
                          {activeDropdown === `tool-${tool.id}` && (
                            <div className="dropdown-menu">
                              <div
                                className="dropdown-item"
                                onClick={() => editGeneralTool(tool.id)}
                              >
                              ...
                              <div
                                ...
                                onClick={() => removeGeneralTool(tool.id)}
                              >
          ...
          <div className="section-header">
            ...
            <div className="add-button-container">
              <button
                ...
                className="btn-secondary add-button"
                onClick={() =>
                  setShowAddSoftwareDropdown(!showAddSoftwareDropdown)
                }
              >
              ...
              {showAddSoftwareDropdown && (
                <div className="dropdown-menu">
                  {AVAILABLE_SOFTWARE.filter(
                    (software) =>
                      !softwareIntegrations.find((s) => s.id === software.id)
                  ).map((software) => (
                    <div
                      key={software.id}
                      className="dropdown-item"
                      onClick={() => addSoftwareIntegration(software.id)}
                    >
                      {software.name}
                    </div>
                  ))}
                    ...
                    <div key={software.id} className="table-row">
                      ...
                      <div className="col configuration">
                        <div className="config-status">
                          {software.desktop && (
                            <div className="action-row">
                              <button
                                ...
                                title="Click to open UI training window - Teach the agent how to interact with this software"
                                onClick={() => {
                                  const agentData = {
                                    name: agentName,
                                    description,
                          ...
                          {(software.scripting || software.desktop) && (
                            <div className="status-row">
                              {software.scripting && (
                                <span className="config-badge">Scripting</span>
                              )}
                              {software.desktop && (
                                <span className="config-badge">Desktop</span>
                              )}
                            </div>
                          )}
                        </div>
                          ...
                          {activeDropdown === `software-${software.id}` && (
                            <div className="dropdown-menu">
                              <div
                                className="dropdown-item"
                                onClick={() =>
                                  editSoftwareIntegration(software.id)
                                }
                              >
                                <FontAwesomeIcon icon={faPen} />
                                <span>Edit</span>
                              ...
                              <div
                                className="dropdown-item danger"
                                onClick={() =>
                                  removeSoftwareIntegration(software.id)
                                }
                              >
                                <FontAwesomeIcon icon={faTrash} />
                                <span>Delete</span>
